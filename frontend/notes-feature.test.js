...
test("测验入口统一收纳在课程侧栏的实践评测分组", () => {
  assert.match(html, /class="course-nav-label">实践评测<\/span>[\s\S]*?data-view="practice">随堂测验<\/button>[\s\S]*?data-view="mistakes">错题复习<\/button>[\s\S]*?data-view="exam">综合考试<\/button>/);
