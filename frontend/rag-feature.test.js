...
test("课程资料库提供成熟上传、选择、状态和全文问答验证界面", () => {
  ...
  assert.match(app, /request\("\/api\/sources\/ask"/);
  assert.match(app, /LLM 全文问答/);
  assert.match(app, /data\.usedCitationIds/);
  assert.match(app, /服务端 LLM/);
  assert.match(app, /ragPipelineStatusLabel/);
  assert.match(app, /data\.pipeline/);
});
...
test("课程生成、导师问答和已有课程绑定均携带真实资料范围", () => {
  ...
  assert.match(server, /\/api\/sources\/ask/);
  assert.match(server, /answerGroundedQuestion/);
